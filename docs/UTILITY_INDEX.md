...
| DeepBook USDC price history | `read.get_deepbook_usdc_price_history` | Implemented | Reads external precomputed `deepbook-usdc-index` 10-minute UTC candle files. Not a utility script, live quote, USD value, P&L, route recommendation, transaction-building input, or signing readiness. |
| DeepBook USDC price at time | `read.get_deepbook_usdc_price_at_time` | Implemented | Selects the filled external DeepBook USDC 10-minute UTC candle for or nearest to one target UTC time. Not a utility script, live quote, USD value, P&L, route recommendation, transaction-building input, or signing readiness. |
