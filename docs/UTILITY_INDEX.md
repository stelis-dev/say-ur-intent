...
| DeepBook USDC candle chart page | `/charts/deepbook-usdc` | Implemented | Local read-only browser page served by the review server. Displays official DeepBookV3 Indexer USDC-denominated candles through same-origin local chart APIs. Not an MCP tool, live feed, wallet page, trading page, USD value page, P&L page, route recommendation, transaction-building input, or signing readiness. |
...
- The local DeepBook USDC candle chart page at `/charts/deepbook-usdc` is a read-only browser page over same-origin chart APIs. It is not an MCP tool, direct browser fetch to the official Indexer, local price database, auto-refresh feed, live quote, trading interface, USD value page, user-account history, P&L, cost basis, route recommendation, transaction-building input, or signing-readiness signal.
