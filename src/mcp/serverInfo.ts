...
export const SERVER_INSTRUCTIONS = [
  "Say Ur Intent is a mainnet-only Sui DeFi intent evidence toolkit with local review. The MCP layer is a session gateway for local review, wallet identity/settings sessions; it does not execute transactions, request wallet signatures, or return transaction bytes. Supported DeepBook/FlowX swaps may build internal unsigned material, bind a digest, derive evidence/PTB visualization, and let the review page request digest-gated handoff for wallet signing. MCP output has no handoff bytes or readiness. No custody, autonomous trading, fiat cash-out, P&L, peg proof, payment readiness, route/best-price advice, or silent settlement-token selection.",
