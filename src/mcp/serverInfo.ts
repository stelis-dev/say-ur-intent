...
export const SERVER_LIMITATIONS = [
  ...
  "DeepBook USDC price history and price-at-time reads use DeepBookV3 official Indexer candle data for the requested official interval. They are external candle evidence only, not live quotes, historical mid prices, global market prices, fiat USD values, USDC/USD peg guarantees, route recommendations, transaction-building inputs, signing readiness, P&L, cost basis, user-account transaction history, or user-account balance history. Say Ur Intent does not independently recompute those candle values from chain history for the response.",
  "The local /charts/deepbook-usdc page displays DeepBookV3 official Indexer USDC-denominated candles through same-origin local chart APIs. It is read-only and is not an MCP tool, live feed, direct browser fetch to the official Indexer, wallet or account page, trading interface, route recommendation, USD value, P&L, tax, cost basis, signing data, or signing readiness.",
